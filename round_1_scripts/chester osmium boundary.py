...
class Trader:
    ...
    POSITION_LIMITS = {"ASH_COATED_OSMIUM": 80}
    ...
    def _load_osmium_buy_qty_lookup(self) -> dict[int, int]:
        """Load a hardcoded buy-quantity schedule from precomputed backtest data."""

        lookup: dict[int, int] = {}
        data_file = Path("analysis_output/order_counts_per_timestamp.csv")
        ...
        with data_file.open(newline="", encoding="utf-8") as handle:
            ...
            for row in reader:
                ...
                ask_volume_sum = int(row["ask_volume_sum"])
                by_timestamp.setdefault(timestamp, []).append(ask_volume_sum)

        for timestamp, values in by_timestamp.items():
            lookup[timestamp] = min(80, max(values) * 2)

        return lookup

    def _get_osmium_buy_qty_from_history(self, data: dict[str, Any], timestamp: int) -> int:
        """Return the precomputed quantity to buy for a given timestamp."""

        lookup = data.get("osmium_buy_qty_lookup", {})
        if not isinstance(lookup, dict) or not lookup:
            lookup = self._load_osmium_buy_qty_lookup()
            data["osmium_buy_qty_lookup"] = lookup

        return int(lookup.get(timestamp, 0))
    ...
    def run(self, state: TradingState):
        ...
        for product in self.POSITION_LIMITS:
            ...
            if product == "ASH_COATED_OSMIUM" and stop_trade:
                ...
                best_bid, best_ask = self._best_bid_ask(order_depth)
                buy_target_qty = self._get_osmium_buy_qty_from_history(trader_state, state.timestamp)
                live_sell_liquidity = sum(-qty for qty in order_depth.sell_orders.values()) if order_depth.sell_orders else 0
                buy_qty = min(buy_target_qty, live_sell_liquidity, self._allowable_buy(product, temp_position))

                if buy_qty > 0 and best_ask is not None:
                    orders.append(Order(product, int(best_ask), buy_qty))
                    temp_position += buy_qty

                sell_qty = min(buy_qty, self._allowable_sell(product, temp_position))
                if sell_qty > 0:
                    orders.append(Order(product, 999999999999, -sell_qty))
                ...
                if osmium_phi is None:
                    logger.print(
                        ...
                        f"buy_target={buy_target_qty} buy_qty={buy_qty} sell_qty={sell_qty} cycle={cycle_period:.0f} orders={len(orders)}"
                    )
                else:
                    logger.print(
