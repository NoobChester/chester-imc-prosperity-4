...
class Trader:
    """Simple mean-reversion market maker with a small amount of liquidity taking.

    The strategy works in two stages:
    1. estimate a fair value for each product,
    2. take clearly mispriced orders and then place passive quotes around that fair value.

    Trade behavior is mainly controlled by:
    - POSITION_LIMITS: maximum inventory per product
    - take_edge: how far away from fair value an order must be before we cross the spread
    - make_size: how large passive quotes should be
    - _fair_values(): how heavily we trust the current mid price versus a fixed anchor
    """
    ...
    def _update_osmium_fair(self, data: dict[str, Any], mid_price: float) -> float:
        """Track osmium fair as a strict last-N rolling average of mid prices."""

        fair_state = data.get("osmium_fair", {}) if isinstance(data.get("osmium_fair", {}), dict) else {}

        values = fair_state.get("values", [])
        if not isinstance(values, list):
            values = []
        values = [float(v) for v in values]

        rolling_sum = float(fair_state.get("sum", 0.0))

        # Backward-compatibility: bootstrap from old {count, avg} state if present.
        if not values and "avg" in fair_state and "count" in fair_state:
            old_count = max(0, min(int(fair_state.get("count", 0)), self.OSMIUM_FAIR_WINDOW))
            old_avg = float(fair_state.get("avg", 10_000.0))
            if old_count > 0:
                values = [old_avg] * old_count
                rolling_sum = old_avg * old_count

        values.append(float(mid_price))
        rolling_sum += float(mid_price)

        if len(values) > self.OSMIUM_FAIR_WINDOW:
            removed = values.pop(0)
            rolling_sum -= removed

        count = len(values)
        fair_value = rolling_sum / count if count > 0 else 10_000.0
    ...
    def _place_taking_orders(
        ...
        """Cross the spread when prices are clearly better than our fair value.

        A smaller edge makes the strategy more aggressive and increases fills.
        """
    ...
    def _place_making_orders(
        ...
        """Post passive buy and sell quotes around the estimated fair value.

        The quote size is reduced when inventory is already skewed in one direction.
        """
    ...
    def _fair_values(self, state: TradingState) -> dict[Symbol, float]:
        """Estimate fair values using the current mid price and a fixed anchor.

        This keeps the strategy from chasing the market too aggressively when the
        book is noisy or thin.
        """
        ...
        # Current mid estimate for OSMIUM. The 10_000 anchor pulls it back to a stable reference.
        osmium_mid = mids.get("ASH_COATED_OSMIUM", 10_000.0)
        # Current mid estimate for PEPPER. The 11_000 anchor plays the same stabilizing role.
        pepper_mid = mids.get("INTARIAN_PEPPER_ROOT", 11_000.0)
        # If you want PEPPER to react faster, reduce the influence of this anchor.
    ...
    def run(self, state: TradingState):
        ...
        for product in self.POSITION_LIMITS:
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                orders: list[Order] = []
                buy_capacity = self._allowable_buy(product, position)
                _, best_ask = self._best_bid_ask(order_depth)
                ...
                logger.print(
                    f"{product} pos={position} best_ask={best_ask} orders={len(orders)}"
                )
                continue
            ...
            if product == "ASH_COATED_OSMIUM":
                ...
                if osmium_mid is not None:
                    osmium_fair = self._update_osmium_fair(trader_state, osmium_mid)

                buy_trigger = osmium_fair - 3
                sell_trigger = osmium_fair + 3
