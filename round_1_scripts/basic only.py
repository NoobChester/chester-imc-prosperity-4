...
class Trader:
    ...
    def _run_root_l1_accumulation_strategy(
        ...
        # If uninitialized average (0) or this buy is above weighted-average, buy half size.

        if float(best_ask) > root_buy_weighted_avg + 1:
            qty = 1
    ...
    def run(self, state: TradingState):
        ...
        for product in self.POSITION_LIMITS:
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                if position >= self.POSITION_LIMITS["INTARIAN_PEPPER_ROOT"]:
                    result[product] = []
                    continue
                orders, _ = self._run_root_l1_accumulation_strategy(order_depth, position, root_buy_weighted_avg)
                logger.print(
                    f"{product} L1_ACCUMULATION pos={position} root_buy_wavg={root_buy_weighted_avg:.2f} orders={len(orders)}"
                )
